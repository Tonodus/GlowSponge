...
public abstract class GlowBaseGridInventory<C extends GlowSlot> extends GlowBaseInventory<C> implements GridInventory, OrderedContentInventory {
    private final int width;
    private final int height;
    private final C[] contents;

    //lazy inserted, based on need
    private final InventoryRow[] rows;
    private final InventoryColumn[] columns;


    public GlowBaseGridInventory(GlowInventory parent, int width, int height) {
        super(parent);
    ...
    public void setRawContents(ItemStack... contents) {
        ...
        for (int i = 0; i < contents.length; i++) {
            this.contents[i].set(contents[i]);
        }
    }
    ...
    public ItemStack[] getRawContents() {
        ...
        for (int i = 0; i < contents.length; i++) {
            contents[i] = this.contents[i].peek().orNull();
        }
        return contents;
    }
