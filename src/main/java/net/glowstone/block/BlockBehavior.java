...
public interface BlockBehavior {
     ...
     */
    Collection<? extends ItemStack> getDrops(Location block, ItemStack tool);
     ...
     */
    GlowTileEntity createTileEntity(GlowChunk chunk, int cx, int cy, int cz);
     ...
     */
    boolean canPlaceAt(Location block, Direction against);
     ...
     */
    void placeBlock(GlowPlayer player, Location location, GlowBlockState state, Direction face, ItemStack holding, Vector3d clickedLoc);
     ...
     */
    void afterPlace(GlowPlayer player, Location block, ItemStack holding);
     ...
     */
    boolean blockInteract(GlowPlayer player, Location block, Direction face, Vector2d clickedLoc);
     ...
     */
    void blockDestroy(GlowPlayer player, Location block, Direction face);
     ...
     */
    boolean canAbsorb(Location block, Direction face, ItemStack holding);
     ...
     */
    boolean canOverride(Location block, Direction face, ItemStack holding);
     ...
     * @param face The Direction to the changed block, or null if unavailable
     ...
     */
    void onNearBlockChanged(Location block, Direction face, Location changedBlock, BlockType oldType, byte oldData, BlockType newType, byte newData);
     ...
     */
    void onBlockChanged(Location block, BlockType oldType, byte oldData, BlockType newType, byte data);

    /**
     * Called when the BlockType should calculate the current physics.
     * @param me The block
     */
    void updatePhysics(Location me);

    void performPlace(GlowPlayer player, Location target, Direction face, ItemStack holding, Vector3d clickedLoc);
}
